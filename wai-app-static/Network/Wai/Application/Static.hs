...
checkPieces _ pieces _
    | any (T.null . fromPiece) $ safeInit pieces =
  ...
  where
    ...
    lookupResult = do
        ...
        case nonIndexResult of
            ...
            _ -> do
                eIndexResult <-
                    lookupIndices (map (\index -> dropLastIfNull pieces ++ [index]) ssIndices)
...
staticAppPieces ss rawPieces req sendResponse = liftIO $ do
  ...
  where
    ...
    response (Redirect pieces' mHash) = do
        let loc =
                ssMkRedirect ss pieces' $
                    L.toStrict $
                        toLazyByteString (H.encodePathSegments $ map fromPiece pieces')
